...
export const PlayPage = (props: History) => {
  ...
  const [plant, setPlant] = useState<PlantType>(plants[0]);
  ...
  useEffect(() => {
    ...
    let selectedPlantName =
      window.localStorage.getItem("selectedPlant") || "White Daisy";
    let selectedPlant = plants.filter(
      (plant) => plant.title === selectedPlantName
    )[0];
...
`;
  ...
  return (
    <div className="flex items-center justify-start flex-col content-around w-full gap-32  h-screen ">
